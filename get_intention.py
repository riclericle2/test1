...
ORG_EMAIL = "@gmail.com" 
FROM_EMAIL = "nqtradenotification" + ORG_EMAIL 
...
def read_email_from_gmail(sc):
    ...
    try:
        ...
        # -------------------- 分割線 --------------------
        # 讀取資料庫中的email_id
        email_id_in_database = pd.read_sql("select * from nqtrade.email_id",con = engine)
        ...
        if latest_email_id > email_id_in_database:
          ...
          for i in range(latest_email_id,email_id_in_database, -1):          
              ...
              for response_part in data:
                  ...
                  if isinstance(arr, tuple):
                      ...
                      if email_subject=='over_three_price_limited':
          ...
          new = email_content["d2"].str.split('\r\n', n = 1, expand = True) 
          email_content["c3"]= new[0] 
          email_content["d3"]= new[1]
          email_content2=email_content[['c','c1','c2','c3']]
          email_content2.columns=['日期','成交量','中户意向','价钱']
          # 上傳更新的數據到Database
          print(email_content2)
          email_content2.to_sql('medium_direction', con = engine, if_exists = 'append', chunksize = 1000, index=False)
          ...
          if latest_email_id > email_id_in_database:
            ...
            update_email_id.to_sql('email_id', con = engine, if_exists = 'append', chunksize = 1000, index=False)
